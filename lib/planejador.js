...
export function syncRetroativo(materias = [], aulas = []) {
  ...
  // 2. Day plans com blocos de estudo → study blocks (caminho inverso)
  // Varre os últimos 90 dias + próximos 30 dias
  let studyBlocks = loadStudyBlocks()
  let studyChanged = false
  const studyIndex = new Set(studyBlocks.map((b) => `${b.materiaId}-${b.date}-${b.startTime}`))
  ...
  for (let i = -90; i <= 30; i++) {
    ...
    plan.blocks.forEach((block) => {
      ...
      const key = `${block.materiaId}-${dk}-${block.startTime}`
      if (studyIndex.has(key)) return

      const sb = {
        id: `retro-${block.uid || block.uid}`,
        date: dk,
        materiaId: block.materiaId,
        topic: block.note || '',
        startTime: block.startTime,
        endTime: block.endTime,
        completed: !!plan.completed?.[block.uid],
        note: block.note || '',
      }
      studyBlocks = [sb, ...studyBlocks]
      studyIndex.add(key)
      studyChanged = true
    })
  }

  if (studyChanged) set('sdv2-study-blocks', studyBlocks)

  // 3. Aulas → day plans para os próximos 14 dias + hoje
  for (let i = -1; i <= 14; i++) {
    const d = new Date(today)
    d.setDate(today.getDate() + i)
    const dk = d.toISOString().slice(0, 10)
