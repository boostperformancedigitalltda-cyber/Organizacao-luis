...
// ── Recurring blocks ──────────────────────────────────────────────────────────
const KEY_RECURRING = 'sdv2-recurring-blocks'

export function loadRecurringBlocks() {
  return get(KEY_RECURRING, [])
}

export function saveRecurringBlocks(blocks) {
  set(KEY_RECURRING, blocks)
}

export function addRecurringBlock(blocks, block, dows) {
  const rb = {
    id: uid(),
    dows, // array of dow 0-6 (0=Sun)
    ...
    icon: block.icon,
    note: block.note || '',
    materiaId: block.materiaId || null,
    planoId: block.planoId || null,
  }
  const updated = [...blocks, rb]
  saveRecurringBlocks(updated)
  return updated
}

export function removeRecurringBlock(blocks, id) {
...
/**
 * Returns recurring blocks for a given date (as day plan blocks).
 */
export function getRecurringForDate(date) {
  const dow = new Date(date + 'T12:00:00').getDay()
  const recurring = loadRecurringBlocks()
  return recurring
    .filter((rb) => rb.dows.includes(dow))
    .map((rb) => ({
      uid: uid(),
